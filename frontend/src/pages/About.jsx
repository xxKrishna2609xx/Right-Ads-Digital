...
export default function About() {
  return (
    <main>
        ...
        <div className="container-main">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '56px', alignItems: 'center', marginBottom: '72px' }}>
            <motion.div initial={{ opacity: 0, x: -35 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', marginBottom: '20px', color: 'var(--text-heading)' }}>
                Our <span className="gradient-text">Story</span>
              </h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', color: 'var(--text-muted)', lineHeight: 1.8, fontSize: '0.95rem' }}>
                <p>Right Ads brings you tailored solutions to meet your needs and budget. We have <strong style={{ color: 'var(--text-primary)' }}>11 years experience in digital services</strong>. Our promise is to provide an enthusiastic team to deliver maximum return with best results for your business.</p>
                ...
                <p>The company offers services to <strong style={{ color: 'var(--text-primary)' }}>Corporate Companies, SME, MSME</strong> and Entrepreneurs.</p>
              </div>
            </motion.div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                {[
                  ...
                  <div key={title} className="card-hover" style={{
                    ...
                    <h4 style={{ color: 'var(--text-heading)', fontWeight: 700, fontSize: '0.9rem', marginBottom: '4px' }}>{title}</h4>
                    <p style={{ color: 'var(--text-faint)', fontSize: '0.8rem', lineHeight: 1.5 }}>{desc}</p>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {[
              ...
              <motion.div key={title}
                ...
                <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-heading)', marginBottom: '10px' }}>{title}</h3>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', lineHeight: 1.75 }}>{desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA ── */}
      <section style={{ padding: '64px 0', textAlign: 'center', borderTop: '1px solid var(--border-subtle)', background: 'var(--bg-highlight)' }}>
        <div className="container-main">
          ...
          <p style={{ color: 'var(--text-muted)', marginBottom: '28px' }}>Let's discuss how Right Ads can help you achieve your digital goals.</p>
